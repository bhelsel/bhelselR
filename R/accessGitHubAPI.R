...
authorize_github_app <- function(
  ...
  saveRDS(
    token,
    file = paste0(directory, "/tokens/", token$app$appname, "_token.RData")
  )
  return(token)
}
...
get_repo_views <- function(token.pathname, user, repo, ...) {
  ...
  if (is.null(dim(data))) {
    message(sprintf(
      "No data found after the GitHub API request for the %s repository",
      repo
    ))
  } else {
...
get_repo_clones <- function(token.pathname, user, repo, ...) {
  ...
  token <- readRDS(token.pathname)
  url <- sprintf(
    "https://api.github.com/repos/%s/%s/traffic/clones",
    user,
    repo
  )
  request <- httr::GET(url, token)
