...
cor_matrix <- function(dataset, vars, var.names = NULL, type = "spearman", 
                       p.value = 0.05, colors = c("#B2BDED", "#C6E0B4")){
  ...
  plot <- 
    ...
    scale_fill_gradient2(low=colors[1], mid="#ffffff", high=colors[2], 
                         midpoint=0, limit=c(-1,1), space="Lab", 
                         name=paste0(toupper(substr(type, 1, 1)), substr(type, 2, nchar(type)), "\nCorrelation")) +
    theme_minimal() +
    coord_fixed() +
    geom_text(aes(Var2, Var1, label = value, fontface = "bold"), color = "black", size = 4) +
