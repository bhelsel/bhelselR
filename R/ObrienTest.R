...
obrien_ranksum <- function(dataset, group, vars, highlow) {
  ...
  ranked.data$ranked.sums = rowSums(ranked.data[
    -(1:(length(ranked.data) - length(vars)))
  ])
  ranked.data = ranked.data[, c("ranked.sums", group)]

  # T-test to determine t statistic
  t.ranked <- stats::t.test(
    ranked.data[, 1] ~ ranked.data[, 2],
    var.equal = TRUE,
  ...
  ranked.data.sum <- cbind(
    group = groupnames,
    rank.sum = as.vector(rowsum(
      ranked.data[, "ranked.sums"],
      ranked.data[, group]
    ))
  )
  ...
  obrien_ranksum_summary <- list(
    "Variables and Order" = o,
