...
find_table <- function(path, tag, label){
  path %>%
    rvest::read_html() %>%
    rvest::html_element(xpath = sprintf("//%s[contains(., '%s')]", tag, label)) %>% 
...
extract_reference_table <- function(path, years = NULL, return_data_frame = FALSE){
  
  if(is.null(years)) years <- as.numeric(format(Sys.Date(), "%Y"))

  labels <- c("Articles (Peer-Review Published)", "Manuscripts in Press", "Manuscripts Submitted")
  
  reference_list <- 
    lapply(1:length(labels), FUN = function(x){
      path %>%
        find_table(tag = "h3", label = labels[x]) %>%
        dplyr::mutate(extracted_years = gsub("[()]", "", stringr::str_extract(X1, "\\(\\d{4}\\)")), 
...
extract_poster_table <- function(path, years = NULL, return_data_frame = FALSE){
  
  if(is.null(years)) years <- as.numeric(format(Sys.Date(), "%Y"))
  
  labels <- c("Oral Paper Presentation", "Poster Presentation", "Invited Seminars at Other Universities")
  
  reference_list <- 
    lapply(1:length(labels), FUN = function(x){
      path %>%
        find_table(tag = "h3", label = labels[x]) %>%
        dplyr::mutate(extracted_dates = gsub("[()]", "", stringr::str_extract(X1, "\\(.*?(\\d{4})\\)")), 
